...
class DeleteCategory extends Component {
  ...
  showPromiseConfirm = (category_id, event) => {
    console.log(this)
    ...
    confirm({
      ...
      content: [<WarningTwoTone twoToneColor="red"/>,<span style={{fontSize:"12px",fontWeight:"700"}}> 삭제시 <span style={{color:"red"}}>영구 삭제</span> 되오니 신중히 결정 하십시오.</span>,
                                                    <br/>,<span style={{fontSize:"11px",fontWeight:"700"}}>- 다른 카테고리를 선택하지 않을시 포함된 책 또한 함께 삭제됩니다.</span>,<br/>,<span style={{fontSize:"11px",fontWeight:"700"}}>- 구매한 책은 미지정 카테고리로 이동되며, 본인이 직접 만든 책은 영구삭제 됩니다.</span>,
                                                    <br/>,<div className="selection_move_down" style={{fontSize:"11px",fontWeight:"700"}}><span>포함된 책을</span> <Select defaultValue="카테고리 선택" size="small" style={{ fontSize:'11px', width: 160 }} onChange={this.handleChange}>
                                                    {optionList}
  ...
  render() { 
    return ( 
      <Space>
        <DeleteOutlined onClick={()=>this.showPromiseConfirm(this.state.category, this.deleteThis)} style={{fontSize:'14px'}} />
      </Space>
    );
  }
}
